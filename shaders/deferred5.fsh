...
vec4 AxialGaussianBlur(){
	...
	ivec2 sampleCoord = clamp(ivec2(texelCoord.x, texelCoord.y - 3), shadowTexBorder.xy, shadowTexBorder.zw);
	blur.r += texelFetch(colortex2, sampleCoord, 0).r * 0.054543;

	sampleCoord = clamp(ivec2(texelCoord.x, texelCoord.y - 2), shadowTexBorder.xy, shadowTexBorder.zw);
	blur.r += texelFetch(colortex2, sampleCoord, 0).r * 0.099384;

	sampleCoord = clamp(ivec2(texelCoord.x, texelCoord.y - 1), shadowTexBorder.xy, shadowTexBorder.zw);
	...
	sampleCoord = clamp(ivec2(texelCoord.x, texelCoord.y + 3), shadowTexBorder.xy, shadowTexBorder.zw);
	blur.r += texelFetch(colortex2, sampleCoord, 0).r * 0.054543;
